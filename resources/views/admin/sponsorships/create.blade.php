    ...
    <div class="form-group">
     ...
     {{--    @foreach ($sponsorships as $sponsorship)
        <div class="form-check form-check-inline">
            <input {{ in_array($sponsorship['id']) }} value="{{ $sponsorship['id'] }}"
                id="{{ 'tag' . $sponsorship['id'] }}" type="checkbox" name="sponsorship[]" class="form-check-input">
            <label for="{{ 'sponsorship' . $sponsorship['id'] }}"
                class="form-check-label">{{ $sponsorship['service_name'] }}</label>
        </div>
        @endforeach --}}
        <div class="form-group">
