...
@section('content')
    ...
    <form action="{{route('admin.apartments.store')}}" method="post" class="text-dark">
        ...
        <div class="mb-3 form-group" >
            ...
            <input name="address" type="text" class="form-control" id="address" placeholder="Inserisci l'indirizzo" value="{{ old('address') }}">
        </div>

        <div class="mb-3 form-group" >
            <label for="city" class="form-label"><h4>Città</h4></label>
            <input name="city" type="text" class="form-control" id="city" placeholder="Inserisci la città" value="{{ old('city') }}">
        </div>
        ...
        <div class="form-group">
            ...
            @foreach ($services as $service)
                <div class="form-check form-check-inline">
                    ...
                    <label for="{{ 'service' . $service['id'] }}" class="form-check-label d-flex align-items-center">{{ $service['service_name'] }} <h4></h4> </label>
                </div>
            @endforeach
        </div>
